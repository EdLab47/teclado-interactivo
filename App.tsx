...
import {
  ...
  MembraneKeyboardIcon,
  ErgonomicKeyboardIcon,
  WirelessKeyboardIcon,
  TipIcon,
  HistoryIcon,
  ActivityIcon,
  KeyboardIcon,
...
const App: React.FC = () => {
  ...
  const handleSectionHover = (section: SectionName | null) => {
    ...
    const secciones = keyboardInfo?.partes?.secciones ?? [];

    if (section && secciones.length > 0) {
      const sectionData = secciones.find((s) => s.nombre === section);
      setActiveSectionDescription(
        sectionData ? sectionData.descripcion : 'Descripción no encontrada.'
      );
    } else {
  ...
  const getTypeIcon = (iconName: KeyboardType['icono']) => {
    switch (iconName) {
      case 'mechanical':
        return <MechanicalKeyboardIcon className="w-12 h-12 text-indigo-500 mb-3" />;
      case 'membrane':
        return <MembraneKeyboardIcon className="w-12 h-12 text-sky-500 mb-3" />;
      case 'ergonomic':
        return <ErgonomicKeyboardIcon className="w-12 h-12 text-emerald-500 mb-3" />;
      case 'wireless':
        return <WirelessKeyboardIcon className="w-12 h-12 text-rose-500 mb-3" />;
      default:
        return null;
    }
  };
          ...
          <div className="space-y-8">
            <InfoCard title="Introducción al Teclado" icon={<KeyboardIcon className="w-8 h-8" />}>
              <h3 className="text-xl font-bold text-indigo-700 mb-2">
                {keyboardInfo.definicion?.titulo}
              </h3>
              <p>{keyboardInfo.definicion?.descripcion}</p>
              <h3 className="text-xl font-bold text-indigo-700 mt-4 mb-2">
                {keyboardInfo.importancia?.titulo}
              </h3>
            ...
            <InfoCard title="Historia y Evolución" icon={<HistoryIcon className="w-8 h-8" />}>
              <h3 className="text-xl font-bold text-indigo-700 mb-2">
                {keyboardInfo.historia?.titulo}
              </h3>
              <div className="space-y-3 mb-6">
                {(keyboardInfo.historia?.contenido ?? []).map((paragraph, index) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div className="md:col-span-1 space-y-2">
                  {(keyboardInfo.partes?.secciones ?? []).map((section) => (
                    <div
                      key={section.nombre}
                      onMouseEnter={() =>
                        handleSectionHover(section.nombre as SectionName)
                      }
                    ...
                    >
                      <h4 className="font-bold text-indigo-700 text-center sm:text-left">
                        {section.nombre}
                      </h4>
                    </div>
                  ))}
                ...
                <div className="md:col-span-2 flex items-center justify-center p-4 bg-slate-50 rounded-lg min-h-[150px] border">
                  {activeSectionDescription ? (
                    <p className="text-slate-700 text-center transition-opacity duration-300">
                      {activeSectionDescription}
                    </p>
                  ) : (
                    <p className="text-slate-400 font-semibold text-center">
                      Pasa el cursor sobre una sección de la izquierda para leer su descripción
                      técnica.
                    </p>
                  )}
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <InfoCard title="Funcionamiento Técnico" icon={<IdeaIcon className="w-8 h-8" />}>
                <h3 className="text-xl font-bold text-indigo-700 mb-2">
                  {keyboardInfo.comoFunciona?.titulo}
                </h3>
              ...
              <InfoCard
                ...
                icon={<MechanicalKeyboardIcon className="w-8 h-8" />}
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {(keyboardInfo.tipos?.teclados ?? []).map((tipo) => (
                    <div
                      key={tipo.nombre}
                      className="flex flex-col items-center text-center p-3 bg-slate-100 rounded-lg"
                    >
            ...
            >
              <h3 className="text-xl font-bold text-indigo-700 mb-2">
                {keyboardInfo.ciudadania?.titulo}
              </h3>
              <p className="mb-4 italic text-slate-600">
                "{keyboardInfo.ciudadania?.introduccion}"
              </p>
            ...
            >
              <h3 className="text-xl font-bold text-indigo-700 mb-2">
                {keyboardInfo.atajos?.titulo}
              </h3>
              <p className="mb-4 text-sm text-slate-600">
                Domina el teclado para trabajar mejor.
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {(keyboardInfo.atajos?.lista ?? []).map((atajo, idx) => (
                  <div
                    key={idx}
                    className="flex flex-col items-center p-3 bg-white border rounded shadow-sm"
                  >
                    ...
                    <span className="text-xs text-center text-slate-500 leading-tight">
                      {atajo.funcion}
                    </span>
                  </div>
                ))}
            ...
            >
              ...
              <ul className="list-disc list-inside space-y-2 mb-6 bg-slate-100 p-4 rounded-lg">
                {(keyboardInfo.actividades?.lista ?? []).map((activity, index) => (
            ...
            <InfoCard
              title="Mecanografía y Ergonomía"
              icon={<TipIcon className="w-8 h-8" />}
              fullWidth
            >
              <h3 className="text-xl font-bold text-indigo-700 mt-4 mb-2">
                {keyboardInfo.consejos?.titulo}
              </h3>
              <ul className="space-y-3 mb-6">
                {(keyboardInfo.consejos?.tips ?? []).map((tip, index) => (
                  <li key={index} className="flex items-start">
                    ...
                    <span>
                      <span className="font-bold">{tip.titulo}:</span> {tip.consejo}
                    </span>
                  </li>
                ))}
              </ul>
