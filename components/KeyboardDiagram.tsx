...
const Key: React.FC<{ layout: KeyLayout; isHighlighted: boolean; isPracticeKey: boolean }> = ({ layout, isHighlighted, isPracticeKey }) => {
    const baseClasses = 'h-12 flex items-center justify-center rounded-md border font-mono transition-all duration-100 transform active:translate-y-px relative shrink-0 text-sm';
    const sizeClass = layout.size || 'w-12 flex-1';

    let colorClasses;
    if (isPracticeKey) {
        colorClasses = 'bg-indigo-500 border-indigo-700 text-white animate-pulse shadow-xl scale-110 z-10 border-b-2';
    } else if (isHighlighted) {
        const highlightColors = getSectionColors(layout.section);
        colorClasses = `${highlightColors} shadow-lg scale-105 border-b-2`;
    } else {
        colorClasses = 'bg-gradient-to-b from-gray-700 to-gray-900 border-gray-950 border-b-4 text-white hover:from-gray-600 hover:to-gray-800 active:border-b-2';
    }
    ...
    if (keyContent === 'icon:win') content = <WindowsIcon {...iconProps} />;
        ...
        content = (
            <div className="flex flex-col items-center justify-center w-full h-full p-1">
                {layout.shiftKey && <span className="absolute top-1 left-1.5 text-xs font-semibold">{layout.shiftKey}</span>}
                <span className={`font-bold uppercase ${layout.shiftKey ? 'pt-1' : ''}`}>{keyContent}</span>
                {layout.secondaryDisplay && <span className="absolute bottom-1 right-1.5 text-xs font-semibold text-cyan-300">{layout.secondaryDisplay}</span>}
            </div>
        );
    }
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-x-6 gap-y-2">
            {sections.map(section => (
                <div key={section} className="flex items-center">
                    <span className={`w-4 h-4 rounded-sm mr-2 ${getSectionColors(section).split(' ')[0]}`}></span>
...
const KeyboardDiagram: React.FC<KeyboardDiagramProps> = ({ highlightedSection, practiceKey }) => {
    ...
    const renderKey = (keyLayout: KeyLayout) => (
         <Key
            key={keyLayout.key + (keyLayout.display || '')}
            layout={keyLayout}
                ...
                <div className="flex justify-center items-start gap-x-4 min-w-max">
                    ...
                    <div className="flex flex-col gap-y-1">
                        <div className="flex w-full gap-x-1">
                            {KEYBOARD_MAIN_BLOCK[0].map(renderKey)}
                        </div>
                    ...
                    <div className="flex flex-col gap-y-8">
                       <div className="flex w-full gap-x-1">
                          {KEYBOARD_SYSTEM_BLOCK[0].map(renderKey)}
                       </div>
                       <div className="grid grid-cols-3 grid-rows-2 gap-1">
                          {KEYBOARD_NAVIGATION_BLOCK[0].map(renderKey)}
                          {KEYBOARD_NAVIGATION_BLOCK[1].map(renderKey)}
                       </div>
                       <div className="grid grid-cols-3 grid-rows-2 gap-1 w-40">
                          <div className="col-start-2">{renderKey(KEYBOARD_ARROWS_BLOCK[0][0])}</div>
                          {KEYBOARD_ARROWS_BLOCK[1].map(renderKey)}
                       </div>
                    </div>

                    {/* Numpad Block */}
                    <div className="grid grid-cols-4 grid-rows-5 gap-1">
                        {KEYBOARD_NUMPAD_BLOCK[0].map(k => <div className="col-span-1">{renderKey(k)}</div>)}
                        
                        <div className="col-span-1 row-span-2">{renderKey(KEYBOARD_NUMPAD_BLOCK[1][3])}</div>
                        {KEYBOARD_NUMPAD_BLOCK[1].slice(0,3).map(k => <div className="col-span-1">{renderKey(k)}</div>)}
                        {KEYBOARD_NUMPAD_BLOCK[2].map(k => <div className="col-span-1">{renderKey(k)}</div>)}
                        
                        <div className="col-span-1 row-span-2">{renderKey(KEYBOARD_NUMPAD_BLOCK[3][3])}</div>
                        {KEYBOARD_NUMPAD_BLOCK[3].slice(0,3).map(k => <div className="col-span-1">{renderKey(k)}</div>)}

                        <div className="col-span-2">{renderKey(KEYBOARD_NUMPAD_BLOCK[4][0])}</div>
                        <div className="col-span-1">{renderKey(KEYBOARD_NUMPAD_BLOCK[4][1])}</div>
                    </div>
                </div>
            </div>
            <Legend />
